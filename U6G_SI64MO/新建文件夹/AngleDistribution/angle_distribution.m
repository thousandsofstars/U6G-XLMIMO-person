...
%% 获取功率角度谱100*2*700
for i = 1:8
    data = load('ang_gain_64_30_45L_1_100.mat');
    ang_gain_8 = data.ang_gain_64_30_45L_1_100;
    ang_gain = squeeze(ang_gain_8(i,:,:,:));
    ...
    figure(1)
    subplot(2,4,i),plot(APAP(:,1),APAP_nor);
end 
...
initialGuess = [0.76, 91, 0.7085];% 确定初值
...
% APSD_lap = 1/(2*a)*exp(-abs(APAP(:,1)-b)/a);  % Laplace 分布
APSD_lap = c/((sqrt(2*pi))*a)*exp(-sqrt(2)*abs(APAP(:,1)-b)/a);
%% Von Mises distribution fitting
fitEquation = fittype('exp(a*cos(pi*(x-b)/180))/(2*pi*besselj(0, a))', 'coefficients', {'a', 'b'});  % Laplace 分布
lowerGuess = [1.067, 89.96];
upperGuess = [3.538, 92.04];
initialGuess = [0.00092, 91];% 确定初值
...
APSD_von=exp(a*cos(pi*(APAP(:,1)-b)/180))/(2*pi*besselj(0, a));  % Laplace 分布
% figure()
